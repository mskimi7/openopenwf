...
void SaveLaunchSettings(const std::wstring& warframeExePath, const std::wstring& langCode, const std::wstring& voLangCode, bool isDx11)
{
	std::wstring settingFileContents = std::format(L"{}*{}*{}*{}", warframeExePath, langCode, voLangCode, isDx11 ? 1 : 0);
...
std::optional<LaunchSettings> LoadLaunchSettings()
{
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		if (ReadFile(hFile, result.data(), fileSize, &readBytes, nullptr))
		{
			...
			settings.voLangCode = settingFileContents[2];
			settings.isDx11 = settingFileContents[3].empty() || settingFileContents[3][0] != L'0';
